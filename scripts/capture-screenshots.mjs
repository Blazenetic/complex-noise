...
async function main() {
  ...
  // ------------------------------------------------------------------
  {
    ...
    // Transform — shorten the mean dwell for this capture, then wait on the
    // renderer's own diagnostics clock rather than guessing a wall-clock delay.
    await page.click('[data-nerd-view="live"]');
    await page.evaluate(() => {
      const dwell = document.getElementById('fieldDwell');
      dwell.value = '4';
      dwell.dispatchEvent(new Event('input'));
    });
    await until(
      page,
      `window.complexNoiseStill.getFieldStats().labelMode === 'transform'`,
      45000,
      'expected the global callout schedule to reach transform'
    );
    await waitForCallouts(page, 3);
    await page.waitForTimeout(500);
    await capture(page, 'info-layer-transform.png');
