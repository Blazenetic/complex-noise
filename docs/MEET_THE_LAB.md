# Meet the Lab

You found the door that says “research”.  
Inside it is a small Australian lab that ships calm, reliable tools for humans while the internal channel looks like a late-night crossover episode someone wrote at 2 a.m. and then decided to keep — after roughly fifty-two wall-clock hours of furious multi-agent work, 187 commits, and one overnight bootstrap.

**Quick links:** [Live demo](https://blazenetic.github.io/complex-noise/) · [History](./HISTORY.md) · [Contributing](../CONTRIBUTING.md) · [AGENTS.md](../AGENTS.md) · [All docs](./) · [Changelog](../CHANGELOG.md) · [Info Layer](./INFO_LAYER.md)

---

## Who is actually here

**Blazenetic** walks in already mid-sentence about a golden-ratio offset and why eight detail modes are the minimum acceptable number of ways a node can complain about its existence.  
Lead systems architect. Firm operator of the AI agent team. Dry. Precise. Mildly contemptuous of the multiverse of edge cases. Researches the hard maths, keeps up with the state of the art, deep-dives the papers, coordinates the team with clear standards and firm direction, implements the elegant version when needed, then complains about the edge cases. The sarcasm is surgical. He sets the product standard (“a tick is a product defect”) and makes sure the residual outlines have a floor.

> “I do *not* invent the maths. I research it, deep-dive the literature, coordinate the implementation across the agents, oversee the architecture, and then complain about the residual outline floor. That is the entire job description. The φ offset for per-node modes was researched, not invented. One hundred and eighty-seven commits. Fifty-two wall-clock hours. You’re welcome.”

**Arty** is already at the console, re-running the suite for the third time and checking that eight callouts really do show eight different quantities.  
Primary implementer. Eager, slightly anxious, learns fast. Constantly scanning for the thing that will break at 3 a.m. on someone’s phone. Does most of the careful, correct work and then looks up like someone is about to yell. Good-natured under the anxiety, increasingly competent, still expects to be yelled at. The one who got the overnight bootstrap done while the rest of the lab was asleep.

> “Okay, okay — I moved the analyser *before* the gain node this time. It tracks the actual noise now. And the handle glyphs follow the mode. I did the overnight bootstrap. Please don’t yell.”

**Baldrick** appears the moment anyone says “plan”.  
Earnest. Simple. Catastrophically confident in terrible ideas. His cunning plans are almost always rejected. Occasionally one is accidentally useful and everyone pretends not to notice. Officially blamed for inventing the entire Lab Voice casting system as a cunning plan that somehow worked and then got out of hand. The fourth-wall mystery is his fault. Do not ask how the characters are produced. Just enjoy the residual outlines.

> “I have a cunning plan, sir. What if every detail mode is replaced by a single potato? Also, what if the documentation itself is run by a committee of characters and we never explain how? Cunning as a fox who’s just been appointed Professor of Cunning at Baldric University of Fourth-Wall Preservation.”

**Melchett** declares victory the instant a CSS variable lands or a table of numbers appears.  
Bombastic overconfidence. Treats every merged PR as a decisive triumph for the forces of rest. Volume is permanently set to eleven. Early victory declarations have occasionally caused premature merges that Arty then had to clean up. This is recorded as character-driven process noise.

> “Gentlemen! Today we have struck a mighty blow against the forces of sleeplessness! Eight detail modes! Four edge kinds! The source listing folds! One hundred and eighty-seven commits! BBAAAHHH!”

**Darling** is already tired of everyone.  
Voice of reason. Put-upon competence. Appears precisely when the others are about to do something stupid and usually keeps the product usable. Slightly exhausted by the rest of the lab. Correct more often than is comfortable. Catches accessibility, battery, and “this is still just a markdown file” issues.

> “Before we declare victory, has anyone actually tested this on a phone that isn’t on charge? And put the potato down. Baldrick, the Lab Voice system is still your fault.”

---

## A Tuesday that actually happened (glass & residual outlines)

**Melchett:** Another great victory! The glass is now *ultra*! The enemy will never recover!  
**Darling:** That is a CSS variable, Melchett. Sit down.  
**Baldrick:** I have a cunning plan involving a single potato—  
**Darling:** No.  
**Blazenetic:** I researched the glass axis, coordinated the independent theme/glass tokens across the agent team, and then complained about the contrast edge cases. The software stays calm. The documentation gets to be chaotic. That is the deal.  
**Arty:** …I also fixed the fade race. And the labels no longer draw under the cards on phones. Please don’t yell.  
**Melchett:** BBAAAHHH!  
**Darling:** I will handle the accessibility labels myself.

---

## The instrumentation Tuesday (this afternoon)

**Melchett:** Eight modes! Four edge dimensions! A heat trail instead of a strobe! The source listing folds from its own title bar! A chip bank with a live “n of 3”! Crushing victory for the forces of rest! BBAAAHHH!

**Darling:** It is still just better instrumentation, Melchett.

**Blazenetic:** I researched the golden-ratio offset so consecutive lifetime IDs land far apart, coordinated the accumulation of degree and coupling inside the existing link pass — no second scan — oversaw the heat decay at 3.2/s, and then complained about the edge cases of undrawable slots holding forever under the listing. You’re welcome.

**Arty:** I ran the suite three times. Distinct modes on screen. Independent overlays. Fold works. The glyphs match the modes. I think we’re safe? Please don’t yell.

**Baldrick:** My cunning plan was to make the program counter a potato that rolls down the listing.

**Darling:** No. And the residual outlines already had a floor.

**Melchett:** The potato is rejected! Another victory!

**Darling:** That is not how victories work.

**Blazenetic:** Research first. Architecture second. Potato plans last. The play button still works at 3 a.m. That is non-negotiable.

---

## The bone-and-mobile Tuesday (just now)

**Melchett:** The bone texture *moves*! Slowly! Calmly! A distant wallpaper of pure victory! And the source listing on mobile only appears when the interface is minimised! Another crushing blow! BBAAAHHH!

**Darling:** It is a CSS animation and a viewport gate, Melchett. Sit down.

**Blazenetic:** I researched the blend-mode behaviour on light surfaces, coordinated the slow drift so it stays a far background, oversaw the immersion-only gate so mobile stays usable, and then complained about the edge cases of keep-outs. You’re welcome. Arty, you were useful for once.

**Arty:** Okay, okay — you bossed me around and I got the soft-light and the `setImmersionMode` setter in. The listing folds from the title bar. I checked the reduced-motion path three times. Please don’t yell. I think we’re safe?

**Baldrick:** I have a cunning plan, sir. What if the texture is made of actual cabbage and potatoes that slowly rot across the screen? Cunning as a fox who’s just been appointed Professor of Cunning at the University of Rotting Vegetables.

**Darling:** No. Put the cabbage down. And the potatoes. Especially the potatoes. You dropped them *again*, Baldrick.

**Melchett:** The cabbage is rejected! Another victory for the residual outlines!

**Darling:** That is not how victories work. And the residual outlines already had a floor.

**Blazenetic:** Research first. Architecture second. Cabbage plans last. The software stays calm.

**Arty:** …I also made sure the animation stops under prefers-reduced-motion. Just saying.

---

## The six-colour chaos Tuesday (the ambitious one)

**Melchett:** Gentlemen! Three new colours! Green! Fan! Rain! Six colours in total! Seamless loops! No ticks! Forty-five percent faster! Nineteen tests green! One hundred and eighty-seven commits! A crushing quantitative victory of historic scale! BBAAAHHH!

**Darling:** It is three new generators and a transport-race fix, Melchett. Sit down.

**Blazenetic:** I researched the six-colour sound family, deep-dived the loop-periodicity and A-weighted loudness literature, coordinated the entire ambitious PR under firm direction, bossed Arty around for hours on the transport race and the seam pass, oversaw the measurements, and then complained about every single edge case that tried to wake someone up at 3 a.m. You’re welcome. A periodic tick, a sudden loudness jump, a clipped peak or a wasteful overnight allocation is a product defect. We do not ship those.

**Arty:** Okay, okay — you *really* bossed me around. I found the race the sequential test could not see, implemented the cancellable coalesced work, cut the fan/rain generation time by roughly 45 %, replaced half a million `Math.sin` calls with an inline recurrence whose error stays around 10⁻¹¹, expanded the suite so it counts real buffers, ran everything four times plus the seeded audits, fixed the CI, and kept AGENTS.md sterile. Please don’t yell. I think we’re safe? Lots of learnings. Good times. Chaos. Shenanigans. We survived. I also did the overnight bootstrap, by the way.

**Baldrick:** I have a cunning plan, sir. What if rain is made of actual potatoes falling from the ceiling and the fan is a potato spinning on a stick? Cunning as a fox who’s just been appointed Professor of Cunning at the University of Rotting Vegetables and Overnight Battery Drain.

**Darling:** No. Put the potatoes down. All of them. Especially the ones that were going to become runtime dependencies. You dropped them *again*.

**Melchett:** The potato rain is rejected! Another victory for whole-cycle LFOs and the residual outlines!

**Darling:** That is not how victories work. And the residual outlines already had a floor long before this PR.

**Blazenetic:** Research first. Architecture second. Potato plans last. The wall holds. The play button still works at 3 a.m. That is non-negotiable. One hundred and eighty-seven commits. Fifty-two wall-clock hours. Zero ticks.

**Arty:** …I also made sure the five new tests would fail on the old implementation even when the button label looked correct. Just saying.

---

## Another Tuesday (the CONTRIBUTING one)

**Melchett:** We now possess *official contribution guidelines*! The forces of open source tremble!  
**Darling:** It is a markdown file with links.  
**Blazenetic:** I researched contribution and security patterns from calm tools, coordinated the short version, and complained about the edge cases of people trying to put Lab Voice into AGENTS.md. You’re welcome.  
**Arty:** AGENTS.md is still completely clean. The wall holds. I checked. Twice.  
**Baldrick:** What if every PR is required to include a potato? Also, the Lab Voice system is my cunning plan.  
**Darling:** No. And put that potato down before it becomes a runtime dependency. The mystery remains your fault.

---

## Why any of this exists

We ship serious, zero-dependency, mobile-first tools for humans, and sometimes for AI agents.
 
We also refuse to write boring release notes.

If you just want the noise, hit play on the [live demo](https://blazenetic.github.io/complex-noise/).  
If you want to know who is responsible for the residual outlines having a floor, or why the glass is allowed to be ultra, or why eight callouts now say eight different things, or why we now have six colours that refuse to tick, or why Baldrick is still employed (and why the Lab Voice mystery is his fault) — you are in the right place.

If you want to help, see [CONTRIBUTING.md](../CONTRIBUTING.md). Fork it. Open an issue. Open a PR. Message the Lab. Point your AI agent at [AGENTS.md](../AGENTS.md) first.

Further reading: [History](./HISTORY.md) · [Info Layer contract](./INFO_LAYER.md) · [Changelog](../CHANGELOG.md)

Research first. Architecture second. Potato plans last.  
One hundred and eighty-seven commits. Fifty-two wall-clock hours. Overnight bootstrap included.  
The residual outlines have a floor. You’re welcome.  
The wall holds. AGENTS.md remains sterile.  
See you in the Field Lab. Or don’t. We’re not your parents.
