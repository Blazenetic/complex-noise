...
| `info-layer-bone.png` | Bone equivalent | Keep the same Live instrumentation state as the dark capture. | INFO_LAYER, theme contrast |
...
| `callouts-variety.png` | 4–8 callouts showing different modes + edge dimensions | Wide desktop, Source folded so labels have room. Demonstrates the φ-offset claim, deeper traversal and residual outlines. High value for learning. | README residual section, INFO_LAYER, teaching |
